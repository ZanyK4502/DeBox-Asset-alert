	...
	expectedPlans := []plans.Plan{
		{
			...
			AllowedRuleTypes: []string{"balance_change", "incoming", "outgoing", "balance_threshold"},
			AllowedRules:     ruleTypes[:4], PrivateNotification: true,
		...
		},
		{
			...
			AllowedRuleTypes: []string{"balance_change", "incoming", "outgoing", "balance_threshold", "approval_change"},
			AllowedRules:     ruleTypes[:5], PrivateNotification: true, DailySummary: true,
...
func expectedRuleTypes() []plans.RuleType {
	return []plans.RuleType{
		...
		{Code: "balance_threshold", Label: "余额阈值", Description: "余额首次达到或低于阈值时提醒一次；持续低于不重复，恢复后再次跌破会重新提醒。"},
