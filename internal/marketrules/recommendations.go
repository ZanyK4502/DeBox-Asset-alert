...
func RecommendThresholds(
	...
	for _, preset := range []struct {
		...
		result = append(result,
			...
			Recommendation{plans.MarketLargeBuy, preset.name, moneyString(trade), "usd", 0, preset.cooldown},
			Recommendation{plans.MarketLargeSell, preset.name, moneyString(trade), "usd", 0, preset.cooldown},
