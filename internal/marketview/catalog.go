...
var marketRuleDefinitions = []RuleDefinition{
	...
	rule(plans.MarketLargeBuy, "单笔大额买入", "Single large buy", "单笔买入达到阈值时提醒。", "Alert when a single buy reaches the threshold.", []string{"usd", "token", "percent"}, "usd", "1000", 0, true),
	rule(plans.MarketLargeSell, "单笔大额卖出", "Single large sell", "单笔卖出达到阈值时提醒。", "Alert when a single sell reaches the threshold.", []string{"usd", "token", "percent"}, "usd", "1000", 0, true),
